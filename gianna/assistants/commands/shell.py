...
class ShellCommand(AbstractCommand):
    ...
    def __init__(
        self, name: str, human_companion_name: str, text_to_speech: SpeechCommand
    ):
        self.name = name
    ...
    def execute(self, prompt: str, **kwargs):
        shell_command_prompt = ShellCommandPrompt(
            self.name, self.human_companion_name, prompt
        ).content
    ...
    def _talk(self, completion_chain_processor, response):
        completion_response = completion_chain_processor.invoke(
            {"command_to_run": response}
        ).output
